...
/**
* Load node-pty lazily (desktop adaptation). The native module must match
* the running runtime's ABI — under the desktop carrier (Electron) an
* npm-built copy throws at require time. Loading is deferred to the first
* spawn so a missing or ABI-mismatched node-pty never takes down the whole
* plugin: explorer / editor / git / browser keep working and the terminal
* surfaces a clear error instead. Each open retries the require, so a
* background Electron rebuild that finishes later is picked up without a
* plugin reload.
*/
function loadNodePty() {
	try {
		...
		throw new SidebarError("pty-error", `terminal unavailable: node-pty failed to load (${error instanceof Error ? error.message : String(error)}); on the desktop app the module must provide a prebuilt binary for this platform`, 503);
	}
}
/**
	...
	*/
	open(sessionId, tabId, cwd, cols, rows) {
		...
		const handle = {
			...
			tabId,
			cwd,
			pty: loadNodePty().spawn(this.shell, shellSpawnArgs(), {
				name: "xterm-256color",
...
var AgentPtyRegistry = class {
	...
	*/
	create(sessionId, title, command, cwd, cols = 80, rows = 24) {
		...
		const pty = loadNodePty().spawn(this.shell, shellSpawnArgs(), {
			name: "xterm-256color",
			cols: dims.cols,
			rows: dims.rows,
			cwd,
